...
let calculate_page_ranks [n] (links:[]link) (ranks: *[n]f64) (sizes:[n]i32) : []f64 =
  ...
  let page_contributions_flags = filter (\(from, c, flag) -> flag) indiced_contributions
  let (page_froms, page_contributions) = unzip (map (\(from, c, flag) -> (from, c)) page_contributions_flags)
...
let calculate_ranks [n] (links:[]link) (ranks_in: *[n]f64) (sizes:[n]i32) (iterations:i32) : [n]f64 =
  --loop ranks_next = ranks_in for i < iterations do
    let ranks_pages = calculate_page_ranks links ranks_in sizes
    in calculate_dangling_ranks ranks_pages sizes

let main (links_array:[][2]i32, sizes:[]i32): []f64 =
  let links = map (\l -> (l[0], l[1])) links_array
  let n_links = length links
  let n_pages = length sizes
  let ranks_initial = replicate n_pages (1f64 / (f64.i32 n_pages))
  --in calculate_page_ranks links ranks_initial sizes
